...
class Admin_functions extends MX_Controller{
	...
	public function contract()
	{
		if($this->uri->segment(4) == 'add'){
			...
			redirect(site_url().'module_contract/admin/proses_kontrak_utama/');
	...
	public function contract_sign()
	{
		...
		if($Q1->num_rows()){
			...
			$link = site_url('module_contract/admin/ttd_digital/'.$contract_id);
	...
	public function contract_signed()
	{
		...
		redirect(site_url('module_contract/admin/ttd_digital/'.$this->uri->segment(4).'/signed'));
	}
	...
	public function contract_import()
	{
		...
		if($this->upload->do_upload('file_excel')){
			...
			redirect(site_url().'module_contract/admin/impor/');
		}
	...
	public function process()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_proses/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__process';
			$datato['where'] = array(
	...
	public function request()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_permintaan/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__request';
			$datato['where'] = array(
	...
	public function request_description()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_detail_permintaan/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__request_description';
			$datato['where'] = array(
	...
	public function third_party()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_pihak_ketiga/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__third_party';
			$datato['where'] = array(
	...
	public function document()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$datato['request_description_id'] = urldecode($this->input->post('request_description_id'));
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_dokumen/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__document';
			$datato['where'] = array(
	...
	public function template_file()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_template/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
	...
	public function user_reviewer()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_user_reviewer/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
	...
	public function reminder()
	{
		if($this->uri->segment(4) == 'add'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__reminder';
			$datato['where'] = array(
				'patlog__contract.entity__reminder.reminder_code' => urldecode($this->input->post('reminder_code')),
				'patlog__contract.entity__reminder.reminder_day' => urldecode($this->input->post('reminder_day'))
			);
			$Q1 = $this->view->view_data($datato);
			if(!$Q1->num_rows()){
				unset($datato);
				$datato['database'] = 'patlog__contract';
				$datato['table'] = 'entity__reminder';
				$datato['reminder_code'] = urldecode($this->input->post('reminder_code'));
				$datato['reminder_day'] = urldecode($this->input->post('reminder_day'));
				$datato['reminder_insert'] = date('Y-m-d H:i:s');
				$this->mod->insert($datato);
			}
			
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_contract/admin/data_pengingat/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__contract.entity__reminder';
			$datato['where'] = array(
				'patlog__contract.entity__reminder.reminder_id' => $this->uri->segment(5)
			);
			$Q1 = $this->view->view_data($datato);
			if($Q1->num_rows()){
				$R1 = $Q1->row();
				
				$reminder_code = strtolower(str_replace(' ','_',urldecode($this->input->post('reminder_code'))));
				unset($datato);
				$datato['table'] = 'patlog__contract.entity__reminder';
				$datato['where'] = array(
					'patlog__contract.entity__reminder.reminder_code' => $reminder_code,
					'patlog__contract.entity__reminder.reminder_day' => urldecode($this->input->post('reminder_day'))
				);
				$Q2 = $this->view->view_data($datato);
				if(!$Q2->num_rows()){
					...
					$datato['table'] = 'entity__reminder';
					$datato['reminder_day'] = urldecode($this->input->post('reminder_day'));
					$datato['field'] = 'reminder_id';
					$datato['id'] = $R1->reminder_id;
					$this->mod->update($datato);
				}
			}
	...
	public function cog()
	{
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_contract/admin/data_konfigurasi/');
	}
	
	public function functions_document_temporary()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', 0);
		set_time_limit(0);
		
		if(urldecode($this->input->post('action')) == 'rollback'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(4));
			$contract_id = $this->encrypt->decode($decrypt_id);
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_contract/admin/fungsi_dokumen_temporary/');
	}
	
	public function functions_inject()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', 0);
		set_time_limit(0);
		...
		if($Q1->num_rows()){
			...
			if(isset($_FILES['contract_summary_file_ttd'])){
				unset($data);
				foreach($_FILES['contract_summary_file_ttd'] as $key => $file){
					$data[$key] = $_FILES['contract_summary_file_ttd'][$key];
				}
				$ext = pathinfo($data['name'], PATHINFO_EXTENSION);
				$file_name = 'document-contract-summary-ttd-'.md5($R1->contract_id).'.'.$ext;
				$path = './assets/mod__contract/attach/document-contract-summary-ttd/'.$file_name;
				$arr_type = array(
					'application/pdf'
				);
				if(in_array($data['type'], $arr_type)){
					...
					$datato['database'] = 'patlog__contract';
					$datato['table'] = 'entity__contract';
					$datato['contract_summary_file_ttd'] = $file_name;
					$datato['field'] = 'contract_id';
					$datato['id'] = $R1->contract_id;
					$this->mod->update($datato);
				}
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_contract/admin/fungsi_inject/');
	}
	
	public function functions_to_locket()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', 0);
		set_time_limit(0);
		...
		$rand = $this->func_rand_string(10);
		$file_name = 'functions-to-locket-'.date('Y-m-d').'-'.$rand.'.xlsx';
		$config['upload_path'] = './assets/mod__contract/attach/temporary/';
		$config['file_name'] = $file_name;
		$config['allowed_types'] = 'xlsx';
		$config['overwrite'] = TRUE;
		...
		if($this->upload->do_upload('file_excel')){
			...
			$this->session->set_flashdata('danger', $this->upload->display_errors());
			redirect(site_url().'module_contract/admin/fungsi_ke_loket/');
		}
		...
		for($row=2;$row<=$highestRow;$row++){
			...
			if($rowData[0][0] != ''){
				...
				$datato['table'] = 'patlog__contract.entity__contract';
				$datato['where'] = array(
					'patlog__contract.entity__contract.contract_no' => $rowData[0][0],
					// 'patlog__contract.entity__contract.contract_status_done' => 'no',
					// 'patlog__contract.entity__contract.contract_status_delete' => 'no'
				);
				$Q1 = $this->view->view_data($datato);
				if($Q1->num_rows()){
					...
					// $datato['database'] = 'patlog__contract';
					// $datato['table'] = 'entity__contract';
					// $datato['contract_date_loket'] = $R1->contract_date_loket;
					// $datato['contract_active_start_date'] = $R1->contract_active_start_date;
					// $datato['contract_active_end_date'] = $R1->contract_active_end_date;
					// $datato['contract_ttd_place'] = $R1->contract_ttd_place;
					// $datato['contract_ttd_date'] = $R1->contract_ttd_date;
					// $datato['contract_no_fix'] = $R1->contract_no_fix;
					// $datato['contract_document_temporary'] = $R1->contract_document_temporary;
					// $datato['contract_summary_file_name'] = $R1->contract_summary_file_name;
					// $datato['contract_summary_file_ttd'] = $R1->contract_summary_file_ttd;
					// $datato['contract_summary_file_final'] = $R1->contract_summary_file_final;
					// $datato['contract_ttd_sign_type'] = $R1->contract_ttd_sign_type;
					// $datato['contract_ttd_sign_speciment'] = $R1->contract_ttd_sign_speciment;
					// $datato['contract_ttd_sign_trigger'] = $R1->contract_ttd_sign_trigger;
					// $datato['contract_ttd_sign_token'] = $R1->contract_ttd_sign_token;
					// $datato['contract_ttd_sign_url'] = $R1->contract_ttd_sign_url;
					// $datato['contract_ttd_sign_link'] = $R1->contract_ttd_sign_url;
					// $datato['contract_ttd_sign_download'] = $R1->contract_ttd_sign_download;
					// $datato['contract_ttd_sign_date_sign'] = $R1->contract_ttd_sign_date_sign;
					// $datato['contract_ttd_sign_date_expired'] = $R1->contract_ttd_sign_date_expired;
					// $datato['contract_ttd_sign_status'] = $R1->contract_ttd_sign_status;
					// $datato['contract_ttd_sign_callback'] = $R1->contract_ttd_sign_status;
					// $datato['contract_ttd_sign_webhook'] = $R1->contract_ttd_sign_webhook;
					// $datato['contract_ttd_sign_webhook_type'] = $R1->contract_ttd_sign_webhook_type;
					// $datato['contract_approval_current_id'] = $R1->contract_approval_current_id;
					// $datato['contract_approval_current_name'] = $R1->contract_approval_current_name;
					// $datato['contract_approval_current_category'] = $R1->contract_approval_current_category;
					// $datato['contract_approval_current_sign'] = $R1->contract_approval_current_sign;
					// $datato['contract_approver_level'] = $R1->contract_approver_level;
					// $datato['contract_approver_message'] = $R1->contract_approver_message;
					// $datato['contract_status_drafter'] = $R1->contract_status_drafter;
					// $datato['contract_status_done'] = 'no';
					// $datato['contract_status_delete'] = 'no';
					// $datato['field'] = 'contract_id';
					// $datato['id'] = $R1->contract_id;
					...
					// unset($datato);
					// $datato['table'] = 'patlog__contract_inj.entity__contract_approval';
					// $datato['where'] = array(
						// 'patlog__contract_inj.entity__contract_approval.contract_id' => $R1->contract_id
					// );
					// $Q2 = $this->view->view_data($datato);
					// foreach($Q2->result() as $R2){
						// unset($datato);
						// $datato['database'] = 'patlog__contract';
						// $datato['table'] = 'entity__contract_approval';
						// $datato['contract_approval_employee_id'] = $R2->contract_approval_employee_id;
						// $datato['contract_approval_employee_name'] = $R2->contract_approval_employee_name;
						// $datato['contract_approval_employee_position'] = $R2->contract_approval_employee_position;
						// $datato['contract_approval_employee_position_detail'] = $R2->contract_approval_employee_position_detail;
						// $datato['contract_approval_status'] = $R2->contract_approval_status;
						// $datato['contract_approval_date'] = $R2->contract_approval_date;
						// $datato['field'] = 'contract_approval_id';
						// $datato['id'] = $R2->contract_approval_id;
						// $this->mod->update($datato);
					// }
					...
					$contract_approval_current_category = null;
					$contract_approval_current_sign = null;
					$contract_approver_message = null;
					unset($datato);
					$datato['table'] = 'patlog__contract.entity__contract_approval';
					$datato['where'] = array(
						'patlog__contract.entity__contract_approval.contract_id' => $R1->contract_id
					...
					foreach($Q2->result() as $R2){
						$total = $Q2->num_rows();
						if($R2->contract_approval_status == null){
							if($R2->contract_approval_category != 'Upload'){
								unset($datato);
								$datato['database'] = 'patlog__contract';
								$datato['table'] = 'entity__contract_approval';
								$datato['contract_approval_status'] = 'Approve';
								...
								$datato['id'] = $R2->contract_approval_id;
								$this->mod->update($datato);
								$contract_approver_level = $contract_approver_level + 1;
							
								unset($datato);
								$datato['table'] = 'patlog__contract.entity__contract_log';
								$datato['where'] = array(
									'patlog__contract.entity__contract_log.contract_id' => $R2->contract_id
								);
								$Q3 = $this->view->view_data($datato);
								$contract_log_approver_level = $Q3->num_rows() + 1;
								...
								if($Q3->num_rows()){
									$R3 = $Q3->row();
									$employee_in_code = $R3->employee_in_code;
									$employee_in_name = $R3->employee_in_name;
									$employee_in_position_detail = $R3->employee_in_position_detail;
								}else{
									$employee_in_code = null;
									$employee_in_name = null;
									$employee_in_position_detail = null;
								...
								$datato['contract_log_approver_level'] = $contract_log_approver_level;
								$datato['contract_log_employee_code'] = $employee_in_code;
								$datato['contract_log_employee_name'] = $employee_in_name;
								$datato['contract_log_employee_position_detail'] = $employee_in_position_detail;
								$datato['contract_log_status'] = 'Approved';
								$datato['contract_log_message'] = 'Approved by Admin';
								$datato['contract_log_insert'] = date('Y-m-d H:i:s');
								$this->mod->insert($datato);
							}
						}
						...
						if(($total-1) == $i){
							$contract_approver_message = '<div class="badge badge-primary">Approve</div> &bull; '.$R2->contract_approval_employee_name;
						}
						$i++;
					}
					...
					$datato['contract_approval_current_sign'] = $contract_approval_current_sign;
					$datato['contract_approver_level'] = $contract_approver_level;
					$datato['contract_approver_message'] = $contract_approver_message;
					$datato['contract_status_drafter'] = 'yes';
					$datato['contract_status_done'] = 'no';
					$datato['contract_status_delete'] = 'no';
					$datato['field'] = 'contract_id';
			...
			// $datato['database'] = 'patlog__contract';
			// $datato['table'] = 'entity__contract_log';
			// $datato['field'] = 'contract_log_message';
			// $datato['id'] = 'Dikembalikan ke Loket';
			// $this->mod->delete($datato);
		}
	...
	public function functions_rollback()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', 0);
		set_time_limit(0);
		...
		$datato['table'] = 'patlog__contract.entity__contract';
		$datato['where'] = array(
			'patlog__contract.entity__contract.contract_id' => urldecode($this->input->post('contract_id'))
		);
		$Q1 = $this->view->view_data($datato);
		if($Q1->num_rows()){
			$R1 = $Q1->row();
			...
			$datato['contract_approval_current_sign'] = $contract_approval_current_sign;
			$datato['contract_approver_level'] = $R1->contract_approver_level - 1;
			$datato['contract_approver_message'] = '<div class="badge badge-warning">Back</div> &bull; '.$contract_approval_current_name;
			$datato['contract_date_close'] = null;
			$datato['contract_status_done'] = 'no';
			$datato['field'] = 'contract_id';
			$datato['id'] = $R1->contract_id;
			$this->mod->update($datato);
			...
			$datato['database'] = 'patlog__procurement';
			$datato['table'] = 'entity__request_legal';
			$datato['request_legal_status'] = 'Disetujui';
			$datato['field'] = 'contract_id';
			$datato['id'] = $R1->contract_id;
			$this->mod->update($datato);
			...
			$datato['contract_log_employee_name'] = $contract_approval_current_name;
			$datato['contract_log_employee_position_detail'] = $employee_in_position_detail;
			$datato['contract_log_status'] = 'Back';
			$datato['contract_log_message'] = 'Rollback';
			$datato['contract_log_insert'] = date('Y-m-d H:i:s');
			$this->mod->insert($datato);
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_contract/admin/fungsi_rollback/');
	}
	
	public function functions_archive()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', 0);
		set_time_limit(0);
		...
		$rand = $this->func_rand_string(10);
		$file_name = 'functions-archive-'.date('Y-m-d').'-'.$rand.'.xlsx';
		$config['upload_path'] = './assets/mod__contract/attach/temporary/';
		$config['file_name'] = $file_name;
		$config['allowed_types'] = 'xlsx';
		$config['overwrite'] = TRUE;
		...
		if($this->upload->do_upload('file_excel')){
			...
			$this->session->set_flashdata('danger', $this->upload->display_errors());
			redirect(site_url().'module_contract/admin/fungsi_arsip/');
		}
	...
	{
		if($this->uri->segment(4) == 'functions_to_locket'){
			ini_set('memory_limit', '-1');
			ini_set('max_execution_time', 0);
			set_time_limit(0);
			...
			$excel->getActiveSheet()->setTitle('Template Impor');
			
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="Template Impor.xlsx"');
			header('Cache-Control: max-age=0');
			$write = IOFactory::createWriter($excel, 'Excel2007');
			$write->save('php://output');
		}elseif($this->uri->segment(4) == 'functions_archive'){
			ini_set('memory_limit', '-1');
			ini_set('max_execution_time', 0);
			set_time_limit(0);
	...
	public function get_table_summary_contract_expired()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-left">
					Nomor SPPK<br/>
					<a target="_blank" href="'.site_url('module_contract/admin/arsip_kontrak_utama?view=preview&contract_id='.$contract_id).'">
						<b>'.$R1->contract_no.'</b>
					</a>
					<br/><br/>
					'.$contract_data_code.'
	...
	public function get_table_dashboard_log()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-left">
					Nomor SPPK<br/>
					<a target="_blank" href="'.site_url('module_contract/admin/proses_kontrak_utama?view=preview&contract_id='.$contract_id).'">
						<b>'.$R1->contract_no.'</b>
					</a>
					<br/><br/>
					'.$contract_data_code.'
	...
	public function get_table_process_contract_project()
	{
		...
		foreach($Q1 as $R1){
			...
			';
			$view = '
				<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_contract/admin/proses_kontrak_utama?view=preview&contract_id='.$contract_id).'">
					<i class="fa fa-eye"></i>
				</a>
			';
			$print = '
			...
			if($R1->contract_approver_level == 0){
				...
				$approval = '
					<a class="btn btn-sm btn-primary" target="_blank" href="'.site_url('module_contract/admin/proses_kontrak_utama?view=approval&contract_id='.$contract_id).'">
			...
			if($R1->contract_approver_level < 2){
				if($R1->contract_data_id == null){
					$edit = '
						<a class="btn btn-sm btn-info" target="_blank" href="'.site_url('module_contract/admin/proses_kontrak_utama?view=manipulation&action=edit&contract_id='.$contract_id).'">
	...
	public function get_table_monitoring()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_contract/admin/monitoring?view=preview&contract_id='.$contract_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-default" target="_blank" href="'.base_url('assets/mod__contract/attach/document-contract-in/'.$R1->contract_document_in.'?time='.date('YmdHis')).'">
						<i class="fa fa-print"></i>
	...
	public function get_table_archive_contract_project()
	{
		...
		foreach($Q1 as $R1){
			...
			';
			$view = '
				<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_contract/admin/arsip_kontrak_utama?view=preview&contract_id='.$contract_id).'">
					<i class="fa fa-eye"></i>
				</a>
			';
			$print = '
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_proses?view=preview&process_id='.$process_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_proses?view=manipulation&action=edit&process_id='.$process_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_permintaan?view=preview&request_id='.$request_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_permintaan?view=manipulation&action=edit&request_id='.$request_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_detail_permintaan?view=preview&request_description_id='.$request_description_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_detail_permintaan?view=manipulation&action=edit&request_description_id='.$request_description_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_pihak_ketiga?view=preview&third_party_id='.$third_party_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_pihak_ketiga?view=manipulation&action=edit&third_party_id='.$third_party_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	{	
		...
		$datato['table_join_on'] = array(
			'patlog__contract.entity__document',
			'patlog__contract.entity__request_description'
		);
		$datato['join_id'] = array(
		...
		foreach($Q1 as $R1){
			...
			$row = array();
			$row[] = '
				<b>'.$R1->request_name.'</b><br/>
				'.$R1->request_description_name.'
			';
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_dokumen?view=preview&document_id='.$document_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_dokumen?view=manipulation&action=edit&document_id='.$document_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_template?view=preview&template_id='.$template_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_template?view=manipulation&action=edit&template_id='.$template_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_user_reviewer?view=preview&user_reviewer_id='.$user_reviewer_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_user_reviewer?view=manipulation&action=edit&user_reviewer_id='.$user_reviewer_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	public function get_table_reminder()
	{	
		unset($datato);
		$datato['table'] = 'patlog__contract.entity__reminder';
		$datato['column_order'] = array(
			'patlog__contract.entity__reminder.reminder_code',
			'patlog__contract.entity__reminder.reminder_day',
			null
		);
		$datato['column_search'] = array(
			'patlog__contract.entity__reminder.reminder_code',
		...
		$datato['order'] = array(
			'patlog__contract.entity__reminder.reminder_code' => 'asc'
		);
		$Q1 = $this->view->get_datatables($datato);
		$data = array();
		foreach($Q1 as $R1){
			# Encrypt ID
			$encrypt_id = $this->encrypt->encode($R1->reminder_id);
			$reminder_id = str_replace(array('+', '/', '='), array('-', '_', '~'), $encrypt_id);

			$row = array();
			$row[] = ucwords(str_replace('_',' ',$R1->reminder_code));
			$row[] = $R1->reminder_day;
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_contract/admin/data_pengingat?view=preview&reminder_id='.$reminder_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_contract/admin/data_pengingat?view=manipulation&action=edit&reminder_id='.$reminder_id).'">
						<i class="fa fa-edit"></i>
					</a>
					<a class="btn btn-sm btn-danger delete" data-toggle="modal" data-target="#confirm" id="delete_'.$R1->reminder_id.'">
						<i class="fa fa-trash"></i>
					</a>
				</div>
			';
	...
	// ===== MONITORING KONTRAK dashboard helpers =====
	// contract_no_fix patterns (from prod sample):
	//   'KTR - xxx'  or  '.../KTR/...'         -> Kontrak Utama
	//   'ADD I xxx', 'ADD II xxx', ...         -> Adendum
	//   'SPK-xxx'    or  '.../SPK/...'         -> SPK
	//   'PPHK...'                              -> PPHK
	//   NULL / empty                           -> Draft
	private function _mk_type_case_sql($col = 'c.contract_no_fix')
	{
		return "CASE
			WHEN {$col} REGEXP '^ADD [IVX]+ +' THEN 'Adendum'
			WHEN {$col} LIKE 'PPHK%' OR {$col} LIKE '%PPHK%' THEN 'PPHK'
			WHEN {$col} LIKE 'SPK%'  OR {$col} LIKE '%/SPK/%' THEN 'SPK'
			WHEN {$col} LIKE 'KTR%'  OR {$col} LIKE 'KTR %' OR {$col} LIKE '%/KTR/%' THEN 'Kontrak Utama'
			ELSE 'Draft'
		END";
	}

	// bucket flag for expiry chip filter (matches JS chip data-filter values)
	// default uses COALESCE(active_end_date, date_end) so both overview & listing
	// see the same effective expiry date
	private function _mk_bucket_case_sql($col = 'COALESCE(c.contract_active_end_date, c.contract_date_end)')
	{
		return "CASE
			WHEN {$col} IS NULL THEN 'no_end_date'
			WHEN {$col} < CURDATE() THEN 'expired'
			WHEN DATEDIFF({$col}, CURDATE()) < 60 THEN 'under_2m'
			WHEN DATEDIFF({$col}, CURDATE()) <= 180 THEN '2_to_6m'
			ELSE 'over_6m'
		END";
	}

	private function _mk_status_flag_sql()
	{
		return "CASE
			WHEN c.contract_status_delete = 'yes' THEN 'inactive'
			WHEN COALESCE(c.contract_active_end_date, c.contract_date_end) IS NULL THEN 'inactive'
			WHEN COALESCE(c.contract_active_end_date, c.contract_date_end) < CURDATE() THEN 'expired'
			ELSE 'active'
		END";
	}

	public function api_kontrak_overview()
	{
		$typeCase   = $this->_mk_type_case_sql();
		$bucketCase = $this->_mk_bucket_case_sql();
		$statusCase = $this->_mk_status_flag_sql();

		// 1. Total released = contract_status_done='yes' OR has contract_no_fix set
		$released = $this->db->query("
			...
			WHERE (c.contract_status_delete IS NULL OR c.contract_status_delete != 'yes')
			  AND ((c.contract_status_done = 'yes') OR (c.contract_no_fix IS NOT NULL AND c.contract_no_fix != ''))
		")->row();

		// 2. Total ongoing request = rows in request_employee_sirkulasi that aren't done
		$ongoing_row = $this->db->query("
			SELECT COUNT(DISTINCT s.request_employee_id) c
		...
		$rows = $this->db->query("
			SELECT
				{$typeCase} AS kontrak_type,
				{$bucketCase} AS bucket,
				{$statusCase} AS status_flag,
				COUNT(*) AS c
			FROM patlog__contract.entity__contract c
			WHERE (c.contract_status_delete IS NULL OR c.contract_status_delete != 'yes')
			GROUP BY kontrak_type, bucket, status_flag
		")->result();

		$by_type = array();
		$buckets = array('under_2m'=>0,'2_to_6m'=>0,'over_6m'=>0,'expired'=>0,'no_end_date'=>0);
		$total_all = 0;
		foreach ($rows as $r) {
			$total_all += (int)$r->c;
			if (!isset($by_type[$r->kontrak_type])) {
				$by_type[$r->kontrak_type] = array('active'=>0,'inactive'=>0,'total'=>0);
			}
			$by_type[$r->kontrak_type]['total'] += (int)$r->c;
			if ($r->status_flag === 'active') {
				$by_type[$r->kontrak_type]['active'] += (int)$r->c;
			} else {
				$by_type[$r->kontrak_type]['inactive'] += (int)$r->c;
			}
			if (isset($buckets[$r->bucket])) {
				$buckets[$r->bucket] += (int)$r->c;
			}
		}
	...
	public function api_kontrak_list()
	{
		...
		$where = "(c.contract_status_delete IS NULL OR c.contract_status_delete != 'yes')";
		$whereBucket = '';
		if (in_array($filter, array('under_2m','2_to_6m','over_6m','expired','no_end_date'), true)) {
			$whereBucket = " AND ({$bucketCase}) = " . $this->db->escape($filter);
		}
		...
		$sql = "
			SELECT
				...
				c.contract_no,
				c.contract_no_fix,
				c.contract_request_name        AS request_name,
				c.contract_project_code_name   AS project_name,
				c.contract_third_party_name    AS third_party_name,
				c.contract_creator_employee_in_name     AS pic_name,
				c.contract_creator_employee_in_position AS pic_position,
				c.contract_project_currency    AS currency,
				c.contract_project_cost        AS value,
				c.contract_date_start          AS date_start,
				COALESCE(c.contract_active_end_date, c.contract_date_end) AS date_end,
				CASE
					WHEN COALESCE(c.contract_active_end_date, c.contract_date_end) IS NULL THEN NULL
					ELSE DATEDIFF(COALESCE(c.contract_active_end_date, c.contract_date_end), CURDATE())
			...
			{$whereBucket}
			ORDER BY
				(COALESCE(c.contract_active_end_date, c.contract_date_end) IS NULL),
				COALESCE(c.contract_active_end_date, c.contract_date_end) ASC
			LIMIT " . $limit;
	...
	public function kontrak_export_xls()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', 0);
		set_time_limit(0);
		...
		$sql = "
			SELECT
				...
				c.contract_date_start          AS date_start,
				COALESCE(c.contract_active_end_date, c.contract_date_end) AS date_end,
				CASE WHEN COALESCE(c.contract_active_end_date, c.contract_date_end) IS NULL THEN NULL
					ELSE DATEDIFF(COALESCE(c.contract_active_end_date, c.contract_date_end), CURDATE()) END AS days_remaining,
				{$typeCase}   AS kontrak_type,
				{$statusCase} AS status_flag
			FROM patlog__contract.entity__contract c
			WHERE {$where}
			{$whereBucket}
			ORDER BY COALESCE(c.contract_active_end_date, c.contract_date_end) ASC
		";
		$items = $this->db->query($sql)->result();

		$excel = new PHPExcel();
		$excel->setActiveSheetIndex(0);
		$excel->getActiveSheet()->setTitle('Monitoring Kontrak');

		$headers = array(
			'No','Nomor Kontrak','Nomor Fix','Judul Request','Nama Proyek','Pihak Ketiga',
			'Tipe','PIC','Jabatan PIC','Mata Uang','Nilai','Tanggal Mulai','Tanggal Berakhir','Sisa Hari','Status',
		);
		$col = 0;
		...
		foreach ($items as $r) {
			$c = 0;
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $i++);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->contract_no);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->contract_no_fix);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->request_name);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->project_name);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->third_party_name);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->kontrak_type);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->pic_name);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->pic_position);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->currency);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->value);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->date_start);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->date_end);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->days_remaining);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($c++, $row, $r->status_flag);
			$row++;
		}
		...
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="Monitoring-Kontrak-'.$filter.'-'.date('Ymd-His').'.xlsx"');
		header('Cache-Control: max-age=0');
