...
class Employee_functions extends MX_Controller{
	...
	{	
		if($this->uri->segment(4) == 'add'){
			...
			if($Q1->num_rows()){
				...
				redirect(site_url().'module_procurement/employee/vendor_registrasi/');
			}
			...
			redirect(site_url().'module_procurement/employee/vendor_proses/');
			...
			if($Q1->num_rows()){
				...
				if($Q2->num_rows()){
					$this->session->set_flashdata('danger', 'Data KTP Penanggung Jawab Perusahaan Sudah Terdaftar Pada Data Vendor Lain.');
					redirect(site_url().'module_procurement/employee/vendor_proses?view=manipulation&action=edit&vendor_id='.$this->input->post('vendor_id'));
				}
	...
	public function vendor_import()
	{
		...
		if($this->upload->do_upload('file_excel')){
			...
			redirect(site_url().'module_procurement/employee/vendor_data?view=import');
		}
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_procurement/employee/vendor_data/');
	}
	...
	public function vendor_import_document()
	{
		...
		if($this->upload->do_upload('file_zip')){
			...
			$this->session->set_flashdata('danger', $this->upload->display_errors());
			redirect(site_url().'module_procurement/employee/vendor_data?view=import_document');
		}
	...
	public function request()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/formulir/');
		}elseif($this->uri->segment(4) == 'edit'){
			ini_set('memory_limit', '-1');
			ini_set('max_execution_time', 0);
			set_time_limit(0);
			...
			$this->session->set_flashdata('success', 'Data berhasil diubah.');
			redirect(site_url().'module_procurement/employee/proses_permintaan/');
		}elseif($this->uri->segment(4) == 'delete'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
			$request_id = $this->encrypt->decode($decrypt_id);
	...
	public function request_vendor()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/'.$page.'?view=process&request_id='.$this->uri->segment(5));
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__procurement.entity__request_legal';
			$datato['where'] = array(
			...
			$this->session->set_flashdata('success', 'Data berhasil diubah.');
			redirect(site_url().'module_procurement/employee/'.$page.'?view=process&request_id='.$request_id);
		}elseif($this->uri->segment(4) == 'delete'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function request_edit_process()
	{
		$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(4));
		$request_id = $this->encrypt->decode($decrypt_id);
		...
		$Q1 = $this->view->view_data($datato);
		if(!$Q1->num_rows()){
			$this->session->set_flashdata('danger', 'Permintaan tidak ditemukan.');
			redirect(site_url().'module_procurement/employee/proses_permintaan/');
			return;
		}
		$R1 = $Q1->row();

		if($R1->request_status != 'process_procurement'){
			$this->session->set_flashdata('danger', 'Edit hanya diizinkan saat status Process Procurement.');
			redirect(site_url().'module_procurement/employee/proses_permintaan/?view=process&request_id='.$this->uri->segment(4));
			return;
		}
		...
		if($Q2->num_rows()){
			$R2 = $Q2->row();
			$actor_id = $R2->employee_in_id;
			$actor_name = $R2->employee_in_name;
		}else{
			$actor_id = null;
			$actor_name = 'User';
		}

		$request_note_new = $this->input->post('request_note');
		unset($datato);
		$datato['database'] = 'patlog__procurement';
		$datato['table'] = 'entity__request';
		$datato['request_note'] = $request_note_new;
		$datato['field'] = 'request_id';
		$datato['id'] = $R1->request_id;
		$this->mod->update($datato);

		$arr_request_document_id = $this->input->post('request_document_id');
		if(is_array($arr_request_document_id)){
			for($i=0;$i<count($arr_request_document_id);$i++){
				unset($datato);
				$datato['table'] = 'patlog__procurement.entity__request_document';
				$datato['where'] = array(
				...
				if(!isset($data['name'][$i]) || $data['error'][$i] != 0){
					continue;
				}
				...
				$file_name = 'document-file-'.md5($R1->request_id).'-'.md5($arr_request_document_id[$i]).'.'.$ext;
				$path = './assets/mod__procurement/attach/request-document-file/'.$file_name;
				$arr_type = explode(',', (string)$request_document_mimes);
				if(in_array($data['type'][$i], $arr_type)){
					$this->_archive_request_document_version($arr_request_document_id[$i], $R1->request_id, $request_document_current_file, 'replace', $actor_id, $actor_name, 'employee', null);
					move_uploaded_file($data['tmp_name'][$i], $path);
					unset($datato);
					$datato['database'] = 'patlog__procurement';
					$datato['table'] = 'entity__request_document';
		...
		$datato['request_id'] = $R1->request_id;
		$datato['request_log_level'] = $request_log_level;
		$datato['request_log_name'] = $actor_name;
		$datato['request_log_status'] = 'Diedit';
		$datato['request_log_message'] = 'Edit dokumen pendukung / deskripsi oleh user saat Process Procurement.';
		$datato['request_log_created_date'] = date('Y-m-d H:i:s');
		$this->mod->insert($datato);

		$this->session->set_flashdata('success', 'Dokumen pendukung dan deskripsi berhasil diperbarui.');
		redirect(site_url().'module_procurement/employee/proses_permintaan/?view=process&request_id='.$this->uri->segment(4));
	}
	...
	public function request_submit()
	{
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_procurement/employee/'.$page.'/');
	}
	...
	public function request_archive()
	{
		if($this->uri->segment(4) == 'edit'){
			...
			$this->session->set_flashdata('success', 'Data berhasil diubah.');
			redirect(site_url().'module_procurement/employee/arsip_permintaan/');
		}
	}
	...
	public function report()
	{
		...
		if(urldecode($this->input->post('export_type')) == 'Process'){
			if($cog_division_id == $division_id){
				...
				$datato['where'] = array(
					'patlog__procurement.entity__request.division_id' => $division_id,
					'patlog__procurement.entity__request.request_is_finish' => 0,
					'patlog__procurement.entity__request.request_is_delete' => 0,
					'patlog__procurement.entity__request.request_used_date >= ' => $this->input->post('export_date_start'),
					'patlog__procurement.entity__request.request_used_date <= ' => $this->input->post('export_date_end')
	...
	public function data_category()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_kategori/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_document()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_dokumen/');
		}elseif($this->uri->segment(4) == 'edit'){
			unset($datato);
			$datato['table'] = 'patlog__procurement.data__document';
			$datato['where'] = array(
	...
	public function data_type()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_tipe/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_unit()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_unit/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_item()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_item/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_item_import()
	{
		...
		if($this->upload->do_upload('import_excel')){
			...
			$this->session->set_flashdata('danger', $this->upload->display_errors());
			redirect(site_url().'module_procurement/employee/data_item?view=import');
		}
	...
	public function data_process_proc()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_proses_pengadaan/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_legal()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_badan_hukum/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_kbli()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_kbli/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_kbli_import()
	{
		...
		if($this->upload->do_upload('import_excel')){
			...
			$this->session->set_flashdata('danger', $this->upload->display_errors());
			redirect(site_url().'module_procurement/employee/data_kbli?view=import');
		}
	...
	public function data_kbli_legality()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_legalitas_kbli/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_csms()
	{
		if($this->uri->segment(4) == 'add'){
			...
			$this->session->set_flashdata('success', 'Data berhasil ditambah.');
			redirect(site_url().'module_procurement/employee/data_csms/');
		}elseif($this->uri->segment(4) == 'edit'){
			$decrypt_id = str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(5));
	...
	public function data_cog()
	{
		...
		$this->session->set_flashdata('success', 'Data berhasil diubah.');
		redirect(site_url().'module_procurement/employee/data_konfigurasi/');
	}
	...
	public function get_table_dashboard_request_archive_reject()
	{
		...
		$arr_all = array();
		unset($datato);
		$datato['table'] = 'patlog__hrms.entity__division';
		$datato['where'] = array(
			'patlog__hrms.entity__division.division_type' => 'Other'
		);
		$Q1 = $this->view->view_data($datato);
		foreach($Q1->result() as $R1){
			$arr_all[] = $R1->division_name;
		}
		...
		if($cog_division_id == $division_id or in_array($employee_in_position, $arr_all)){
			...
			$datato['where_in'] = array(
				'patlog__procurement.entity__request.division_id',
				'patlog__procurement.entity__request.request_is_finish',
				'patlog__procurement.entity__request.request_is_delete',
				'patlog__procurement.entity__request_legal.request_legal_status'
			);
		...
		foreach($Q1 as $R1){
			...
			if($R1->request_legal_status == 'Dihapus' or $R1->request_legal_status == 'Ditolak'){
				if($employee_in_id == $R1->request_proc_employee_in_id){
					$button = '
			...
			';
			$view = '
				<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_procurement/employee/arsip_permintaan?view=preview&request_id='.$request_id).'">
					<i class="fa fa-eye"></i>
				</a>
			';
			$process = '';
			...
			if($employee_in_id == $R1->request_proc_employee_in_id){
				$process = '
					<a class="btn btn-sm btn-warning" target="_blank" href="'.site_url('module_procurement/employee/arsip_permintaan?view=process&request_id='.$request_id).'">
						<i class="fa fa-random"></i>
					</a>
				';
				$edit = '
					<a class="btn btn-sm btn-info" target="_blank" href="'.site_url('module_procurement/employee/arsip_permintaan?view=manipulation&action=edit&request_id='.$request_id).'">
	...
	public function get_table_data_vendor_process()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = $vendor_status;
			$view = '
				<a class="btn btn-sm btn-default" href="'.site_url('module_procurement/employee/vendor_proses?view=preview&vendor_id='.$vendor_id).'">
					<i class="fas fa-eye"></i>
				</a>
			';
			if($R1->vendor_level > 0 and $R1->vendor_done == 'no' and $employee_in_id == $R1->vendor_approval_id){
				$approve = '
					<a class="btn btn-sm btn-primary" href="'.site_url('module_procurement/employee/vendor_proses?view=approval&vendor_id='.$vendor_id).'" >
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/vendor_proses?view=manipulation&action=edit&vendor_id='.$vendor_id).'">
					<i class="fa fa-edit"></i>
				</a>
			';
			$delete	= '
	...
	public function get_table_data_vendor()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_procurement/employee/vendor_data?view=history&vendor_id='.$vendor_id).'">
						<i class="fas fa-eye"></i> Lihat
					</a>
				</div>
			';
			$view = '
				<a class="btn btn-sm btn-default" href="'.site_url('module_procurement/employee/vendor_data?view=preview&vendor_id='.$vendor_id).'">
					<i class="fas fa-eye"></i>
				</a>
			';
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/vendor_data?view=manipulation&action=edit&vendor_id='.$vendor_id).'">
					<i class="fa fa-edit"></i>
				</a>
			';
			$delete	= '
	...
	public function get_table_data_vendor_history()
	{
		...
		foreach($Q1 as $R1){
			...
			$row = array();
			$row[] = '
				<a href="'.site_url('module_procurement/employee/arsip_permintaan?view=preview&request_id='.$request_id).'" target="_blank">
					<b>'.$R1->request_code.'</b>
				</a>
			';
	...
	public function get_table_request_process()
	{
		...
		if($functions_id != $cog_functions_id){
			if($division_id == $cog_division_id){
				$where = '
					(patlog__procurement.entity__request.employee_in_id = '.$employee_in_id.' OR 
					patlog__procurement.entity__request.request_approval_employee_in_id = '.$employee_in_id.' OR 
					(patlog__procurement.entity__request.request_process_employee_in_id = '.$employee_in_id.' AND 
					patlog__procurement.entity__request.request_proc_employee_in_id is null) OR 
					patlog__procurement.entity__request.request_proc_employee_in_id = '.$employee_in_id.') AND 
					patlog__procurement.entity__request.request_is_finish = 0 AND 
					patlog__procurement.entity__request.request_is_delete = 0
				';
			}else{
				$where = '
					(patlog__procurement.entity__request.employee_in_id = '.$employee_in_id.' OR 
					patlog__procurement.entity__request.request_approval_employee_in_id = '.$employee_in_id.') AND 
					patlog__procurement.entity__request.request_is_finish = 0 AND 
					patlog__procurement.entity__request.request_is_delete = 0
				';
			}
			...
			$where = '
				(patlog__procurement.entity__request.employee_in_id = '.$employee_in_id.' OR 
				patlog__procurement.entity__request.request_approval_employee_in_id = '.$employee_in_id.' OR 
				patlog__procurement.entity__request.request_approval_employee_in_name = "Loket" OR 
				(patlog__procurement.entity__request.request_process_employee_in_id = '.$employee_in_id.' AND 
				patlog__procurement.entity__request.request_proc_employee_in_id is null) OR 
				patlog__procurement.entity__request.request_proc_employee_in_id = '.$employee_in_id.') AND 
				patlog__procurement.entity__request.request_is_finish = 0 AND 
		...
		foreach($Q1 as $R1){
			...
			';
			$view =	'
				<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_procurement/employee/proses_permintaan?view=preview&request_id='.$request_id).'">
					<i class="fa fa-eye"></i>
				</a>
			';
			$print = '
				<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_procurement/employee_functions/print_request/'.$request_id).'">
					<i class="fas fa-print"></i>
				</a>
			';
			if($R1->request_status == 'waiting_approve' and $employee_in_id == $R1->request_approval_employee_in_id){
				$approve = '
					<a class="btn btn-sm btn-primary" target="_blank" href="'.site_url('module_procurement/employee/proses_permintaan?view=approval&request_id='.$request_id).'">
						<i class="fa fa-check"></i>
					</a>
				';
			}elseif($functions_id == $cog_functions_id and $R1->request_approval_employee_in_name == 'Loket'){
				$approve = '
					<a class="btn btn-sm btn-primary" target="_blank" href="'.site_url('module_procurement/employee/proses_permintaan?view=approval&request_id='.$request_id).'">
						<i class="fa fa-check"></i>
					</a>
			...
			if(($R1->request_approval_level == 1 or $R1->request_status == 'reject') and $employee_in_id == $R1->employee_in_id){
				$edit = '
					<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/proses_permintaan?view=manipulation&action=edit&request_id='.$request_id).'">
						<i class="fa fa-edit"></i>
					</a>
				';
			}else{
			...
			if($R1->request_status == 'process_procurement' and $R1->request_proc_employee_in_id == null and $employee_in_id == $R1->request_process_employee_in_id){
				$mapping = '
					<a class="btn btn-sm btn-warning" target="_blank" href="'.site_url('module_procurement/employee/proses_permintaan?view=mapping&request_id='.$request_id).'">
			...
			if($R1->request_status == 'process_procurement' and $R1->request_proc_employee_in_id != null and $employee_in_id == $R1->request_proc_employee_in_id){
				$process = '
					<a class="btn btn-sm btn-warning" target="_blank" href="'.site_url('module_procurement/employee/proses_permintaan?view=process&request_id='.$request_id).'">
						<i class="fa fa-random"></i>
					</a>
				';
			}else{
			...
			if($R1->vendor_id != null and $employee_in_id == $R1->request_proc_employee_in_id){
				$finish	= '
	...
	public function get_table_monitoring()
	{
		...
		foreach($Q1 as $R1){
			...
			';
			$view = '
				<a class="btn btn-sm btn-default" target="_blank" href="'.site_url('module_procurement/employee/monitoring?view=preview&request_id='.$request_id).'">
					<i class="fa fa-eye"></i>
				</a>
			';
			$print = '
	...
	public function get_table_data_category()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_kategori?view=manipulation&action=edit&category_id='.$category_id).'">
	...
	{	
		...
		foreach($Q1 as $R1){
			...
			$row[] = '
				<div class="text-center">
					<a class="btn btn-sm btn-default" href="'.site_url('module_procurement/employee/data_dokumen?view=preview&document_id='.$document_id).'">
						<i class="fa fa-eye"></i>
					</a>
					<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_dokumen?view=manipulation&action=edit&document_id='.$document_id).'">
						<i class="fa fa-edit"></i>
					</a>
	...
	public function get_table_data_type()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_tipe?view=manipulation&action=edit&type_id='.$type_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_unit()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = $R1->unit_name;
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_unit?view=manipulation&action=edit&unit_id='.$unit_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_item()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_item?view=manipulation&action=edit&item_id='.$item_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_process_proc()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_proses_pengadaan?view=manipulation&action=edit&process_proc_id='.$process_proc_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_legal()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_badan_hukum?view=manipulation&action=edit&legal_entity_id='.$legal_entity_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_kbli()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_kbli?view=manipulation&action=edit&kbli_id='.$kbli_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_kbli_legality()
	{
		...
		foreach($Q1 as $R1){
			...
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_legalitas_kbli?view=manipulation&action=edit&kbli_legality_id='.$kbli_legality_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_table_data_csms()
	{
		...
		foreach($Q1 as $R1){
			...
			$row[] = $R1->csms_name;
			$edit = '
				<a class="btn btn-sm btn-info" href="'.site_url('module_procurement/employee/data_csms?view=manipulation&action=edit&csms_id='.$csms_id).'">
					<i class="fa fa-edit"></i> <span class="hidden-xs"></span>
				</a>
			';		
			$delete	= '
	...
	public function get_modal_cancel()
	{
		...
		$data['isi'] = '
			<form method="post" action="'.site_url('module_procurement/employee_functions/request_cancel/'.$this->input->post('request_id')).'" enctype="multipart/form-data">
	...
	public function get_modal_finish()
	{
		...
		$data['isi'] = '
			...
			<form method="post" action="'.site_url('module_procurement/employee_functions/request_finish/'.$this->input->post('request_id')).'" enctype="multipart/form-data">
				<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'" required="">
				<div class="modal-header">
					<button class="close" data-dismiss="modal">
						&times;
			...
			<script type="text/javascript">
				...
				$("#request_pic_contract_id").change(function(){
					$.ajax({
						...
						url: "'.site_url('module_procurement/employee_functions/get_dropdown_approval_contract/').'",
				...
				$("#request_pic_contract_request_id").change(function(){
					$.ajax({
						...
						url: "'.site_url('module_procurement/employee_functions/get_dropdown_request_description/').'",
						success: function(result){
							var data = JSON.parse(result);
	...
	public function get_modal_submit()
	{
		...
		$datato['where'] = array(
			'patlog__contract.entity__contract.contract_data_id' => $request_id
		);
		$Q1 = $this->view->view_data($datato);
		if($Q1->num_rows()){
			$status = 'true';
		...
		$data['isi'] = '
			...
			<form method="post" action="'.site_url('module_procurement/employee_functions/request_submit/'.$this->input->post('request_legal_id')).'" enctype="multipart/form-data">
				<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'" required="">
	...
	public function func_generate_qr($id)
	{
		...
		if($Q1->num_rows()){
			...
			$encrypt_id = $this->encrypt->encode($R1->request_id);
            $request_id = str_replace(array('+', '/', '='), array('-', '_', '~'), $encrypt_id);
			$params['data'] = site_url('module_procurement/employee/lacak_permintaan/'.$request_id); //data yang akan di jadikan QR CODE
